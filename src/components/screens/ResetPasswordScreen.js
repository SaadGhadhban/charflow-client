...
const ResetPasswordScreen = ({match})=> {
    ...
    return (
        <div className='forgotpassword-screen'>
            <form onSubmit={handleResetPassword} className='forgotpassword-screen__form'>
                <h3 className='forgotpassword-screen__title'>Reset Password</h3>
                {error && <span className='error-message' >{error}</span>}
                {success && <span className='success-message' >{success} <Link className='login-link' to='/register'>Login</Link></span>}
                ...
                {success?'':<button type='submit' className="btn btn-primary">Reset</button>}
